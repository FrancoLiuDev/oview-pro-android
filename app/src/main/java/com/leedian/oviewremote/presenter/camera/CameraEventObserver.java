...
public class CameraEventObserver {
    ...
    public interface ChangeListener {
        ...
        void onfNumberListChanged(ArrayList fNumberList ,String fNumber);
     ...
     * need.
     */
    public abstract static class ChangeListenerImp implements ChangeListener {
    ...
    private CameraRemoteApi mRemoteApi;
     ...
     */
    public CameraEventObserver(Context context, CameraRemoteApi apiClient) {
        if (context == null) {
            ...
            public void run() {
                ...
                MONITORLOOP: while (mWhileEventMonitoring) {
                    ...
                    try {
                        ...
                        Boolean isFocused = findFocusStatus(replyJson);
                        if (isFocused) {
                            
                            fireCameraFocusedListener();
                        }
                        ...
                        ArrayList fNumberList = findFNumberStatus(replyJson);
                        String fNumber = findCurrentFNumber(replyJson);
                        ...
                        if (fNumberList!=null){
                            ...
                            fireAvaliblefNumberChangeListener(fNumberList ,fNumber);
                        }
    ...
    private void fireCameraFocusedListener() {
        mUiHandler.post(new Runnable() {
            @Override
            public void run() {
                if (mListener != null) {
                    mListener.onCameraFocused();
                }
            }
        });
    }
    ...
    /**
     * Notify the change of Live view Status.
     * 
     * @param status
     */
    ...
    private void fireAvaliblefNumberChangeListener(final ArrayList fNumberList ,final String fNumber) {
        mUiHandler.post(new Runnable() {
            @Override
            public void run() {
                if (mListener != null) {
                    mListener.onfNumberListChanged(fNumberList,fNumber);
                }
            }
        });
    }
    ...
    private static Boolean findFocusStatus(JSONObject replyJson) throws JSONException {
        Boolean isFocused  = false;
        int indexOfFocusStatus = 34;
        JSONArray resultsObj = replyJson.getJSONArray("result");
        if (!resultsObj.isNull(indexOfFocusStatus)) {
            JSONObject cameraStatusObj = resultsObj.getJSONObject(indexOfFocusStatus);
            String type = cameraStatusObj.getString("type");
            if ("touchAFPosition".equals(type)) {
                isFocused = true;
                  //isFocused = cameraStatusObj.getBoolean("currentSet");
            } else {
                Log.w(TAG, "Event reply: Illegal Index (1: CameraStatus) " + type);
            }
        }
    ...
    private static ArrayList findFNumberStatus(JSONObject replyJson) throws JSONException {
        String fNumber = null;
        JSONArray avalibleFNumber = null;
        ArrayList list = null;

        int indexOfFNumberStatus = 27;
        JSONArray resultsObj = replyJson.getJSONArray("result");
        if (!resultsObj.isNull(indexOfFNumberStatus)) {
            JSONObject fNumberStatusObj = resultsObj.getJSONObject(indexOfFNumberStatus);
            String type = fNumberStatusObj.getString("type");
            if ("fNumber".equals(type)) {
                fNumber = fNumberStatusObj.getString("currentFNumber");
                avalibleFNumber= fNumberStatusObj.getJSONArray("fNumberCandidates");

                if (avalibleFNumber instanceof JSONArray) {
                    list = JsonConvertUtils.convertJsonToArray(avalibleFNumber);
                }
    ...
    private static String findCurrentFNumber(JSONObject replyJson) throws JSONException {
        String fNumber = null;
